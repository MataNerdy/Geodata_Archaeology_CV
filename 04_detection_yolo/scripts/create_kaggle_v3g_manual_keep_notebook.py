...
OUT = Path("notebooks/kaggle_yolo_v3g_manual_keep.ipynb")
...
cells = [
    md(
        """
        # YOLO v3g Manual Keep Baseline

        One controlled experiment on the manually cleaned Li-only kurgan dataset:

        ```bash
        yolo detect train \\
          model=yolov8n.pt \\
          data=datasets/dataset_yolo_bbox_v3g_li_medium_manual_keep_only/dataset.yaml \\
          imgsz=640 \\
          epochs=100 \\
          batch=16 \\
          seed=42 \\
          ...
          name=v3g_li_manual_keep_yolov8n_640
        ```

        Recommended Kaggle setup: attach a Kaggle Dataset containing the prebuilt
        `dataset_yolo_bbox_v3g_li_medium_manual_keep_only/` folder. This is safer than rebuilding on the fly because v3g depends on manual audit decisions.
        """
    ),
    md("## 1. Configuration"),
    code(
        ...
        REPO_BRANCH = "main"
        REPO_DIR = WORK_ROOT / "Geodata_Archaeology_CV"
        ...
        RUN_NAME = "v3g_li_manual_keep_yolov8n_640"
    ...
    code(
        ...
        def find_file(name: str) -> Path | None:
            candidates = sorted(KAGGLE_INPUT_ROOT.rglob(name))
            return candidates[0] if candidates else None

        source_dir = PREBUILT_DATASET_DIR if PREBUILT_DATASET_DIR.exists() else find_dataset_dir(DATASET_FOLDER)
        ...
        if source_dir is not None:
            ...
            if zip_path is None:
                base_dataset = find_dataset_dir("dataset_yolo_bbox")
                audit_index = find_file("audit_index.csv")
                audit_decisions = find_file("audit_decisions.csv")
                if base_dataset is None or audit_index is None or audit_decisions is None:
                    raise FileNotFoundError(
                        "Attach either a prebuilt "
                        f"{DATASET_FOLDER}/ dataset, {DATASET_FOLDER}.zip, "
                        "or base dataset_yolo_bbox/ plus audit_index.csv and audit_decisions.csv."
                    )

                print("Prebuilt v3g was not found. Building from base dataset and manual audit files.")
                print("Base dataset:", base_dataset)
                print("Audit index:", audit_index)
                print("Audit decisions:", audit_decisions)
                ...
                builder = PROJECT_DIR / "scripts" / "build_v3g_manual_keep_only.py"
                subprocess.run(
                    [
                        sys.executable,
                        str(builder),
        ...
        print(boxes[source_col].value_counts())
        """
    ),
    md("## 4. Train YOLOv8n"),
    code(
    ...
    code(
        ...
        report = [
            "# v3g Manual Keep YOLOv8n 640 Report",
            "",
            "## Config",
            "",
        ...
        report_path = ANALYSIS_DIR / "v3g_manual_keep_report.md"
    ...
    code(
        ...
        archive_path = WORK_ROOT / f"yolo_v3g_manual_keep_{timestamp}.zip"
